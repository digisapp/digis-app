-- =============================================================================
-- FINAL RLS MIGRATION FOR DIGIS APP
-- =============================================================================
-- Current State:
--   - 67 tables with RLS DISABLED
--   - 40 tables have policies (ready for RLS)
--   - 27 tables need policies created
--
-- This script will:
--   1. Create helper functions
--   2. Add performance indexes
--   3. Create policies for remaining 27 tables
--   4. Enable RLS on all tables in stages
...
-- =============================================================================
-- STEP 2: CREATE CRITICAL INDEXES (if not exist)
-- =============================================================================
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_subscriptions_active
  ON subscriptions(creator_id, subscriber_id)
...
RAISE NOTICE '✅ Base permissions granted';
...
-- =============================================================================
-- These 27 tables need policies before we can enable RLS
...
-- application_logs (admin/system only)
...
-- ledger_snapshots (financial - strict access)
...
-- migrations (system table - no access)
...
-- processed_webhooks (system table)
...
CREATE POLICY "View session quality metrics"
  ...
  TO authenticated
  USING (true);  -- Sessions endpoint handles auth
...
RAISE NOTICE '✅ Policies created for 27 tables';

COMMIT;

-- =============================================================================
-- STEP 5: ENABLE RLS - STAGE 1 (Crown Jewels)
-- =============================================================================
...
RAISE NOTICE '✅ STAGE 1 COMPLETE: Core tables (13 tables)';

COMMIT;

-- =============================================================================
-- STEP 6: ENABLE RLS - STAGE 2 (Financial)
-- =============================================================================
...
RAISE NOTICE '✅ STAGE 2 COMPLETE: Financial tables (10 tables)';

COMMIT;

-- =============================================================================
-- STEP 7: ENABLE RLS - STAGE 3 (Content & Creator Features)
-- =============================================================================
...
RAISE NOTICE '✅ STAGE 3 COMPLETE: Content & Creator (14 tables)';

COMMIT;

-- =============================================================================
-- STEP 8: ENABLE RLS - STAGE 4 (Streaming & Classes)
-- =============================================================================
...
RAISE NOTICE '✅ STAGE 4 COMPLETE: Streaming & Classes (14 tables)';

COMMIT;

-- =============================================================================
-- STEP 9: ENABLE RLS - STAGE 5 (Analytics & Metadata)
-- =============================================================================
...
RAISE NOTICE '✅ STAGE 5 COMPLETE: Analytics & Metadata (10 tables)';

COMMIT;

-- =============================================================================
-- STEP 10: ENABLE RLS - STAGE 6 (System Tables)
-- =============================================================================
...
RAISE NOTICE '✅ STAGE 6 COMPLETE: System tables (6 tables)';
...
-- Check for tables with RLS but no policies (DANGEROUS!)
SELECT
  '⚠️ WARNING' as alert,
  pt.tablename,
  'RLS enabled but NO policies - table is locked!' as issue
FROM pg_tables pt
JOIN pg_class c ON c.relname = pt.tablename
JOIN pg_namespace n ON n.oid = c.relnamespace
WHERE pt.schemaname = 'public'
...
-- ✅ Policies created for 27 tables
...
-- 4. Run storage policies (separate script)
-- =============================================================================
