...
router.post('/send', authenticateToken, async (req, res) => {
  try {
    ...
    const { recipientId, content, messageType = 'text', cost = 0 } = req.body;
    ...
    try {
      ...
      if (cost > 0) {
        // Get sender's current token balance
        const balanceQuery = 'SELECT token_balance FROM users WHERE supabase_id = $1';
