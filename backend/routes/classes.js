...
const logger = winston.createLogger({
  ...
  transports: [
    new winston.transports.File({ filename: '../logs/classes.log' }),
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const userResult = await pool.query('SELECT is_creator FROM users WHERE id = $1', [userId]);
...
router.post('/:classId/join', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const userResult = await client.query(
      'SELECT id, id::text as user_id FROM users WHERE id = $1',
      [userId]
    );
