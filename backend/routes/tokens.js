  ...
  try {
    ...
    const balanceResult = await pool.query(
      `SELECT balance, total_purchased, total_spent, total_earned 
       FROM token_balances 
       WHERE supabase_user_id = $1::uuid OR user_id = $1::uuid`,
      [userId]
    );
    ...
    if (balanceResult.rows.length === 0) {
      ...
      await pool.query(
        `INSERT INTO token_balances (supabase_user_id, balance, created_at) 
         VALUES ($1, 0, NOW())`,
        [userId]
      );
...
router.post('/gift', authenticateToken, async (req, res) => {
  const { recipientId, tokenAmount, message = '', giftType = 'standard' } = req.body;
  ...
  try {
    ...
    // Calculate gift fee (2% for platform)
    const platformFee = Math.max(1, Math.floor(tokenAmount * 0.02));
    const netTokens = tokenAmount - platformFee;
    ...
    await client.query(
      `INSERT INTO token_gifts (id, sender_id, recipient_id, token_amount, net_tokens, 
       platform_fee, message, gift_type, status, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'completed', NOW())
       RETURNING *`,
      [giftId, req.user.supabase_id, recipientId, tokenAmount, netTokens, platformFee, message, giftType]
    );

    // Record transactions for both users
    await client.query(
      `INSERT INTO token_transactions (user_id, type, tokens, amount_usd, status, gift_id, created_at)
       VALUES ($1, 'gift_sent', $2, $3, 'completed', $4, NOW())`,
      [req.user.supabase_id, -tokenAmount, tokenAmount * TOKEN_VALUE, giftId]
    );

    await client.query(
      `INSERT INTO token_transactions (user_id, type, tokens, amount_usd, status, gift_id, created_at)
       VALUES ($1, 'gift_received', $2, $3, 'completed', $4, NOW())`,
      [recipientId, netTokens, netTokens * TOKEN_VALUE, giftId]
    );
