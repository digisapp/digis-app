...
router.post('/sync-user', verifySupabaseToken, async (req, res) => {
  try {
    ...
    const checkQuery = `
      SELECT * FROM users 
      WHERE supabase_id = $1 OR email = $2
      LIMIT 1
    `;
    ...
    if (accountType === 'fan') {
      ...
      // Creator username from metadata
      username = metadata?.username || email.split('@')[0].toLowerCase().replace(/[^a-z0-9_]/g, '');
    }
    ...
    if (accountType === 'creator' && metadata?.username) {
      ...
      try {
        ...
        const adminsQuery = await pool.query(
          'SELECT supabase_id FROM users WHERE is_super_admin = true'
        );
        ...
        for (const admin of adminsQuery.rows) {
          ...
          `, [
            ...
            `${displayName} has applied to become a creator`,
            JSON.stringify({
              applicationId,
              username,
              email,
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        (
          SELECT COUNT(*) FROM followers 
          WHERE follower_id = u.supabase_id OR follower_id = u.supabase_id
        ) as following_count
      FROM users u
      LEFT JOIN token_balances tb ON 
        tb.supabase_user_id = u.supabase_id OR 
        tb.user_id = u.supabase_id
      WHERE u.supabase_id = $1 OR u.supabase_id = $2
      LIMIT 1
    `;
  ...
  try {
    ...
    if (username && isCreator) {
      ...
      const checkQuery = `
        ...
        WHERE username = $1 AND (supabase_id != $2 AND supabase_id != $3)
        LIMIT 1
      `;
    ...
    const updateQuery = `
      UPDATE users 
      SET ${updates.join(', ')}
      WHERE (supabase_id = $${paramCount} OR supabase_id = $${paramCount + 1})
      RETURNING *
    `;
...
router.post('/apply-creator', authenticateToken, async (req, res) => {
  try {
    ...
    const userIdValue = req.user.id || userId;
  ...
  try {
    ...
    try {
      ...
      await pool.query(`
        ...
        WHERE member_id IN (SELECT id FROM users WHERE supabase_id = $1 OR supabase_id = $2)
        OR creator_id IN (SELECT id FROM users WHERE supabase_id = $1 OR supabase_id = $2)
      `, [userId, userId]);
      
      // Finally delete the user
      await pool.query('DELETE FROM users WHERE supabase_id = $1 OR supabase_id = $2', [userId, userId]);
