...
router.post('/send', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const creatorResult = await client.query(
      'SELECT id, supabase_id, username, display_name, is_creator FROM users WHERE id = $1',
      [creatorId]
    );
    ...
    await client.query(
      'UPDATE token_balances SET balance = balance - $1, updated_at = NOW() WHERE supabase_user_id = $2',
      [amount, tipperId]
    );
    ...
    const tipResult = await client.query(
      `INSERT INTO tips (tip_id, supabase_tipper_id, creator_id, amount, message, session_id, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW())
       RETURNING *`,
      [tipId, tipperId, creatorId, amount, message, sessionId]
    );

    // Record token transaction for tipper
    await client.query(
      `INSERT INTO token_transactions (
        transaction_id, supabase_user_id, type, amount, balance_after, 
    ...
    // Record token transaction for creator
