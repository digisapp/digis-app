...
router.get('/status', authenticateToken, async (req, res) => {
  try {
    ...
    const subscriptionQuery = `
      ...
      JOIN users u ON (u.id::text = s.user_id::text OR u.supabase_id::text = s.user_id::text)
      WHERE (s.user_id::text = $1::text OR s.user_id IN (
        SELECT id::text FROM users WHERE supabase_id::text = $1::text
    ...
    if (result.rows.length > 0) {
      ...
      const userQuery = 'SELECT tv_trial_used FROM users WHERE supabase_id::text = $1::text OR id::text = $1::text';
      const userResult = await pool.query(userQuery, [userId]);
      
      res.json({
        hasAccess: false,
        subscription: null,
        isTrialAvailable: userResult.rows[0] && !userResult.rows[0].tv_trial_used
      });
    }
  } catch (error) {
...
router.post('/start-trial', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const userQuery = 'SELECT tv_trial_used FROM users WHERE id = $1 FOR UPDATE';
