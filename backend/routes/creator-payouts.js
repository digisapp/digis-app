...
router.get('/dashboard', authenticateToken, requireCreator, async (req, res) => {
  try {
    ...
    const dashboardQuery = `
      SELECT * FROM creator_payout_dashboard WHERE creator_id = $1
    `;
    ...
    const recentPayoutsQuery = `
      ...
      LIMIT 10
    `;
    const recentPayouts = await pool.query(recentPayoutsQuery, [creatorId]);
  ...
  try {
    ...
    const accountQuery = `
      SELECT 
        ...
        u.email,
        u.country
      FROM creator_stripe_accounts sa
      JOIN users u ON u.uid = sa.creator_id
      WHERE sa.creator_id = $1
    `;
...
router.get('/settings', authenticateToken, requireCreator, async (req, res) => {
  try {
    ...
    const query = `
      SELECT * FROM creator_payout_settings WHERE creator_id = $1
    `;
