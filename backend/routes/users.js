...
router.put('/profile', authenticateToken, validate(profileUpdateSchema), async (req, res) => {
  ...
  if (isCreator) {
    ...
    // Industry type is required for creators
    if (!industryType || industryType.trim().length === 0) {
      return res.status(400).json({
        error: 'Industry type is required for creators',
        timestamp: new Date().toISOString()
      });
    }
  }
  ...
  try {
    ...
    if (existingUser.rows.length > 0) {
      result = await client.query(
        `UPDATE users 
         SET is_creator = $2, bio = $3, profile_pic_url = $4, stream_price = $5, video_price = $6, voice_price = $7, message_price = $8, 
             text_message_price = $9, image_message_price = $10, video_message_price = $11, voice_memo_price = $12,
             username = $13, industry_type = $14, show_token_balance = $15, gallery_photos = $16, stream_audience_control = $17, 
             state = $18, country = $19, updated_at = NOW()
         WHERE supabase_id = $1
         RETURNING *`,
        [uid, isCreator, bio, profilePic, streamPrice || 5.00, videoPrice || 8.00, voicePrice || 6.00, messagePrice || 2.00, 
         textMessagePrice || 1.00, imageMessagePrice || 3.00, videoMessagePrice || 5.00, voiceMemoPrice || 2.00,
         username, industryType, showTokenBalance || false, JSON.stringify(galleryPhotos || []), streamAudienceControl || 'public',
         state, country]
      );
      logger.info('✅ Profile updated successfully');
    } else {
      result = await client.query(
        `INSERT INTO users (supabase_id, is_creator, bio, profile_pic_url, stream_price, video_price, voice_price, message_price, 
                           text_message_price, image_message_price, video_message_price, voice_memo_price,
                           username, industry_type, show_token_balance, gallery_photos, stream_audience_control, state, country, created_at, updated_at) 
...
router.get('/profile', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const user = result.rows[0];
    const isOwnProfile = targetUid === req.user.supabase_id;
...
router.post('/upload-profile-image', authenticateToken, uploadImage.single('file'), async (req, res) => {
  ...
  try {
    // For now, we'll store images as base64 URLs in the database
...
router.get('/public/creators', async (req, res) => {
  ...
  try {
    ...
    const countResult = await pool.query(
      'SELECT COUNT(*) as total FROM users WHERE is_creator = TRUE AND username IS NOT NULL'
    );
...
router.get('/creators', async (req, res) => {
  ...
  const allowedSortFields = ['created_at', 'price_per_min', 'total_sessions', 'total_earnings'];
  const allowedSortOrders = ['ASC', 'DESC'];
  ...
  try {
    const result = await pool.query(
      `SELECT 
         id, supabase_id, bio, profile_pic_url, price_per_min, created_at, updated_at,
...
router.post('/session', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const creatorResult = await client.query(
      'SELECT id, supabase_id, price_per_min FROM users WHERE supabase_id = $1 AND is_creator = TRUE',
      [creatorId]
    );
    ...
    const result = await client.query(
      `INSERT INTO sessions (creator_id, member_id, start_time, type, status, price_per_min) 
  ...
  try {
    ...
    const sessionResult = await client.query(
      `UPDATE sessions 
       ...
       WHERE id = $1 AND (creator_id = $2 OR member_id = $2)
       RETURNING *`,
      [sessionId, userId]
    );
    ...
    if (session.member_id === userId && session.type !== 'stream' && tokenAmount > 0) {
      // Get creator info
      const creatorResult = await client.query(
        'SELECT supabase_id FROM users WHERE id = $1',
        [session.creator_id]
      );
...
router.get('/session/:sessionId/billing', authenticateToken, async (req, res) => {
  ...
  try {
    const session = await pool.query(
      'SELECT * FROM sessions WHERE id = $1 AND (member_id = (SELECT id FROM users WHERE supabase_id = $2) OR creator_id = (SELECT id FROM users WHERE supabase_id = $2))',
...
router.get('/public/creator/:username', async (req, res) => {
  const { username } = req.params;
  
  try {
    const result = await pool.query(
      `SELECT username, bio, profile_pic_url, stream_price, video_price, voice_price, message_price, 
              created_at, total_sessions, total_earnings, state, country
       FROM users 
    ...
    const followerCount = await pool.query(
      'SELECT COUNT(*) as count FROM followers WHERE creator_id = (SELECT supabase_id FROM users WHERE username = $1)',
      [username]
    );
