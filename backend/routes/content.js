...
router.get('/creator/:username', async (req, res) => {
  try {
    ...
    const creatorQuery = await pool.query(
      'SELECT id, supabase_id, username, display_name, profile_pic_url, bio, is_creator FROM users WHERE username = $1',
      [username]
    );
    ...
    const contentQuery = await pool.query(`
      ...
      LEFT JOIN content_purchases cp ON c.id = cp.content_id AND cp.user_id = $1
      WHERE c.creator_id = $2 AND c.is_active = true
      ORDER BY c.created_at DESC
    `, [userId, creator.supabase_id]);

    // Separate pictures and videos
    const pictures = contentQuery.rows.filter(item => item.content_type === 'picture');
    const videos = contentQuery.rows.filter(item => item.content_type === 'video');
...
router.post('/purchase', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await client.query(
      'UPDATE token_balances SET balance = balance + $1 WHERE supabase_user_id = $2',
      [content.price * 0.8, content.creator_id] // 80% to creator, 20% platform fee
    );
