...
const EnhancedStreamingOverlay = ({
  ...
  useEffect(() => {
    ...
    const giftInterval = setInterval(() => {
      ...
      const giftData = {
        id: Date.now(),
        emoji: randomGift,
        sender: `Fan${Math.floor(Math.random() * 1000)}`,
        value: Math.floor(Math.random() * 100) + 10
      };
      addGiftAnimation(giftData);
    }, 15000); // Reduced frequency from 8s to 15s
    ...
    const tipInterval = setInterval(() => {
      const tipData = {
        ...
        sender: `Supporter${Math.floor(Math.random() * 1000)}`,
        message: ['Great stream!', 'Love your content!', 'Keep it up!', 'Amazing!'][Math.floor(Math.random() * 4)]
      };
      addTipAnimation(tipData);
    }, 20000); // Reduced frequency from 12s to 20s

    // Simulate viewer reactions
    const reactionInterval = setInterval(() => {
      const reactions = ['❤️', '🔥', '😍', '👏', '✨', '💯'];
      const randomReaction = reactions[Math.floor(Math.random() * reactions.length)];
      addViewerReaction(randomReaction);
    }, 6000); // Reduced frequency from 3s to 6s

    // Simulate new follower
    const followerInterval = setInterval(() => {
      setNewFollower({
        name: `NewFan${Math.floor(Math.random() * 1000)}`,
        timestamp: Date.now()
      });
    }, 30000); // Reduced frequency from 20s to 30s
  ...
  return (
    <div className={`absolute inset-0 pointer-events-none ${className}`}>
      ...
      {/* Digis Logo Watermark - Above Video */}
      <motion.div
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.5 }}
        className="absolute top-0 left-1/2 transform -translate-x-1/2 -ml-10 pointer-events-none z-10"
      >
        <img 
          src="/digis-logo-white.png" 
          alt="Digis" 
          className="h-12 w-auto opacity-60 drop-shadow-lg"
        />
      </motion.div>
