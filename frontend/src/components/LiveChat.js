...
const LiveChat = ({ 
  ...
  useEffect(() => {
    ...
    const initRTM = async () => {
      try {
        ...
        // Login to RTM
        await rtmClient.login({ token: data.rtmToken, uid: user.uid });
        setIsConnected(true);
  ...
  const getUserRole = (userId) => {
    if (userId === user.uid && isCreator) return 'creator';
    if (userId === user.uid && isHost) return 'host';
  ...
  const getUsernameColor = (userRole, userId) => {
    ...
    if (subscription) {
      if (subscription.tier === 'vip' || subscription.tier === 'premium') {
        return 'text-yellow-500 font-semibold'; // Gold for VIP fans
      }
      if (subscription.tier === 'plus' || subscription.tier === 'standard') {
        return 'text-emerald-500 font-semibold'; // Emerald for subscribers
      }
    }
