...
const LiveReactionsSystem = ({ 
  ...
  const sendReaction = (reactionEmoji, count = 1) => {
  ...
  const sendReactionBurst = (reactionEmoji, count = 5) => {
    if (!websocket || !channelId) return;
    ...
    websocket.send(JSON.stringify(burstData));
  };
  ...
  const handleLongPressReaction = (reaction) => {
    // Long press for burst reaction
    sendReactionBurst(reaction.emoji, 5);
  };
...
export { default } from './LiveReactionsSystemEnhanced';
export { default as LiveReactionsSystemOriginal } from './LiveReactionsSystem';