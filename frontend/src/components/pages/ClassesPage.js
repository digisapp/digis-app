...
const ClassesPage = ({ user, isCreator, tokenBalance, onTokenUpdate }) => {
  ...
  const categories = [
    { id: 'all', label: 'All Classes', icon: AcademicCapIcon, color: 'from-purple-500 to-indigo-600' },
    ...
    { id: 'tech', label: 'Technology', icon: AcademicCapIcon, color: 'from-indigo-500 to-purple-600' },
  ...
  // Mock enrolled classes for demo
  useEffect(() => {
    if (user) {
      // In production, fetch from API
      setEnrolledClasses([1, 3, 5]); // Mock: user enrolled in classes with IDs 1, 3, 5
    }
  }, [user]);
    ...
    const mockClasses = [
      {
        ...
        tokenPrice: 15,
        tags: ["Beginner Friendly", "Morning Routine", "Flexibility"],
        isLive: false
      },
      {
        id: 2,
        ...
        tokenPrice: 20,
        tags: ["Advanced", "Cardio", "Strength"],
        isLive: false
      },
      {
        id: 3,
        title: "Fashion Styling Masterclass",
        description: "Learn how to style outfits for different occasions and body types",
        creator: {
          username: "StyleGuru_Emma",
        ...
        tokenPrice: 35,
        tags: ["Styling Tips", "Wardrobe", "Confidence"],
        isLive: false
      },
      {
        id: 4,
        title: "Healthy Meal Prep Workshop",
        description: "Plan and prepare nutritious meals for the entire week",
        creator: {
          username: "Chef_Maria",
        ...
        tokenPrice: 25,
        tags: ["Meal Prep", "Nutrition", "Time Saving"],
        isLive: false
      },
      {
        id: 5,
        title: "Digital Marketing Deep Dive",
        description: "Master social media marketing and content strategy for business growth",
        creator: {
      ...
      },
      {
        ...
        description: "Learn beautiful watercolor techniques for beginners",
        creator: {
          username: "ArtistLily",
      ...
      },
      {
        ...
        description: "Build modern websites with React and Next.js",
        creator: {
          username: "CodeMaster_David",
      ...
      },
      {
        ...
        description: "Learn chords, strumming patterns, and popular songs",
        creator: {
          username: "GuitarGuru_Jake",
        ...
        tokenPrice: 22,
        tags: ["Guitar", "Music Theory", "Songs"],
        isLive: false
      }
    ];
        ...
        <div className="relative overflow-hidden rounded-2xl bg-white shadow-sm hover:shadow-xl transition-all duration-300">
          ...
          <div className="relative h-64 overflow-hidden">
            ...
            <div className="absolute top-4 right-4">
              <span className="px-3 py-1 bg-white/90 backdrop-blur-sm rounded-full text-xs font-medium text-gray-800 flex items-center gap-1">
          ...
          <div className="p-4">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-900">{classItem.creator.displayName}</p>
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => (
                      <StarIconSolid
                        key={i}
                        className={`w-3 h-3 ${i < Math.floor(classItem.creator.rating) ? 'text-yellow-400' : 'text-gray-300'}`}
                      />
                    ))}
                    <span className="text-xs text-gray-600 ml-1">{classItem.creator.rating}</span>
                  </div>
                </div>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-500">{formatDate(classItem.startTime).split(',')[0]}</p>
                <p className="text-xs font-medium text-gray-700">{classItem.currentParticipants} joined</p>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="flex-1 py-2.5 rounded-xl font-medium text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 transition-colors duration-200"
              >
                View Details
              </button>
              {!isEnrolled && (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm text-gray-500">Join interactive sessions with creators</span>
            </div>
            {isCreator && (
              <motion.button
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            {/* Left side - View toggles and filters */}
              ...
              {user && (
                ...
                >
                  ...
                  {showMyClasses && enrolledClasses.length > 0 && (
                    <span className="ml-0.5 bg-purple-600 text-white px-1.5 py-0.5 rounded-full text-xs font-semibold">
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={() => {
                  ...
                  newDate.setDate(newDate.getDate() - (activeView === 'daily' ? 1 : 7));
              ...
              <div className="px-3 py-1.5 text-sm font-medium text-gray-900 min-w-[180px] text-center">
                {activeView === 'daily' 
                  ? formatDate(selectedDate)
                  : `Week of ${formatDate(selectedDate).split(',')[0]}`
                }
              </div>
      ...
      <AnimatePresence mode="wait">
        {filteredClasses.length === 0 ? (
          ...
          >
            ...
            >
              <CalendarDaysIcon className="w-20 h-20 text-gray-300 mb-4" />
            </motion.div>
            <h3 className="text-xl font-semibold text-gray-900 mb-2">No Classes Found</h3>
            <p className="text-gray-600 mb-6 max-w-md text-center">
              No classes scheduled for {activeView === 'daily' ? 'today' : 'this week'}. Check back later or browse upcoming dates!
            </p>
            {isCreator && (
              <motion.button
                whileHover={{ scale: 1.02 }}
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <div>
                ...
                <p className="text-sm text-gray-600">{selectedClassForReviews.title}</p>
              </div>
              <button
                onClick={() => {
        ...
        {showDetailsModal && selectedClassForDetails && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto my-8"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Class Image Header */}
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-semibold text-gray-900">{selectedClassForDetails.creator.displayName}</p>
                      <div className="flex items-center gap-1">
                        {[...Array(5)].map((_, i) => (
                          <StarIconSolid
                            key={i}
                            className={`w-4 h-4 ${i < Math.floor(selectedClassForDetails.creator.rating) ? 'text-yellow-400' : 'text-gray-300'}`}
                          />
                        ))}
                ...
                {selectedClassForDetails.requirements && (
                  <div>
                    ...
                    <p className="text-gray-700">{selectedClassForDetails.requirements}</p>
                  </div>
                )}
                ...
                {selectedClassForDetails.whatToExpect && (
                  <div>
                    ...
                    <p className="text-gray-700">{selectedClassForDetails.whatToExpect}</p>
                  </div>
                )}
