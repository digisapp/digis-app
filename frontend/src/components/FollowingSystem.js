...
const FollowingSystem = ({ user, onCreatorSelect }) => {
  ...
  const fetchFollowingData = async () => {
    try {
      ...
      const [followingResponse, activityResponse] = await Promise.all([
        fetch(`${import.meta.env.VITE_BACKEND_URL}/api/users/following`, {
          headers: { 'Authorization': `Bearer ${authToken}` }
        }),
        fetch(`${import.meta.env.VITE_BACKEND_URL}/api/users/following/activity`, {
          headers: { 'Authorization': `Bearer ${authToken}` }
        })
      ]);
  ...
  const handleUnfollow = async (creatorId) => {
    try {
      const authToken = await getAuthToken();
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/users/unfollow`, {
        method: 'POST',
        headers: {
    ...
    <div className="space-y-4">
      {following.length === 0 ? (
        ...
        following.map(creator => (
          <div key={creator.id} className="bg-white border border-gray-200 rounded-xl p-4 hover:shadow-md transition-shadow">
