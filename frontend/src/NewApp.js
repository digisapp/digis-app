...
const AppContent = () => {
  ...
  const fetchUserProfile = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIsCreator(profile.user?.is_creator || false);
        setIsAdmin(profile.user?.is_super_admin || false);
      }
    } catch (error) {
  ...
  const handleJoinPrivateSession = useCallback((sessionData) => {
    ...
    console.log('Joining private session:', sessionData);
  }, [user, handleSignIn]);
  ...
  const handleCreatorSelect = useCallback((creator, selectedSessionType, callRequestId) => {
    console.log('Creator selected:', creator, 'for', selectedSessionType);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <div className="text-3xl font-bold text-gray-900 mb-2">Digis</div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {showGoLiveSetup && (
        <GoLiveSetup
          ...
          onGoLive={(streamData) => {
            console.log('Going live:', streamData);
